...
\begin{document}
  ...
  We optimize the parameters of each schedule manually. The shape of the schedules are shown in Figure \ref{fig:schedules_linear_shape} and \ref{fig:schedules_discretized_shape}. We then compare them against each other on a graph of size $N=250$ and using $q=15$ colors. The edges density $c$ is linearly increased from 0 to $N/4$. Some results are displayed in Figure \ref{fig:schedules_evaluation}. The discretized schedules all perform better than the continuous ones. The continuous sublinear schedule (here, a square root) is the best continuous schedule and is compared against the discretized schedules. We notice that the logarithmic schedule, also a slow-growing function of time, has similar performance.
