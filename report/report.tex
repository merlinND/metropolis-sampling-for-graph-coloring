...
\begin{document}
  ...
  \begin{figure}[h]
    ...
    \begin{subfigure}[t]{.49\linewidth}
      \centering
      \includegraphics[width=7cm]{figures/random-walk-example.pdf}
      \caption{}\label{Fig:random-walk-example}
    \end{subfigure}
    \begin{subfigure}[t]{.49\linewidth}
      \centering
      \includegraphics[width=7cm]{figures/cost-vs-graph-density.pdf}
  ...
  We define in Table \ref{tab:schedules} five different schedules, each tunable by some parameters. They all depend on the current time step $t \in [0, 1]$, normalized by the maximum number of iterations. The inverse temperature should approach ``infinity'' when $t$ approaches the maximum number of iterations, such that the acceptance probabilities approach $0$ at this time. [The adaptive schedule is similar to the exponential one, except that $\alpha$ is now a function of $t$ (for instance, $\alpha(t) = 1/t$).] Moreover, in each case, $t$ can be replaced by $t-\tau$ in order to keep the temperature constant for at the start of the simulation before starting to increase it.
  ...
  We hand-tune the parameters of each schedule in order to obtain good results. The shape of the schedules are shown in Figure \ref{fig:schedules_shape}. We then compare them against each other on a graph of size $N=250$ and $q=15$ colors. The edges density $c$ is linearly increased from 0 to $N/4$. The results are displayed in Figure \ref{fig:schedules_evaluation}. The ``square root'' schedule performs consistently better than the others. We notice that the logarithmic schedule, also a slow-growing function of time, has similar performance.
