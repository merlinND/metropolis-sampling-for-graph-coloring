...
if isbitmap(options)
    ...
    if options.transparent && (options.png || options.alpha)
        ...
        % Center A and B (bug fix)
        min_size = min(size(A), size(B));
        offset_A = max(round((size(A) - min_size)/2),0);
        offset_B = max(round((size(B) - min_size)/2),0);
        A = A(offset_A(1)+(1:min_size(1)), offset_A(2)+(1:min_size(2)), offset_A(3)+(1:min_size(3)));
        B = B(offset_B(1)+(1:min_size(1)), offset_B(2)+(1:min_size(2)), offset_B(3)+(1:min_size(3)));
